...
public class CollectableMove extends MoveBase {
  ...
  public CollectableMove(
      Direction direction, int distance, GameGeometry map, final Player fullPlayer) {
  ...
  public void phase() {
    if (collectables.size() > 0) {
      ...
      for (Collectable collectable : collectables) {
        if (findDistanceBetweenTwoPositionsSquared(playerPosition, collectable.getPosition())
            <= minDistanceSquared) {
          nearestCollectable = collectable;
          minDistanceSquared =
              findDistanceBetweenTwoPositionsSquared(
                  playerPosition, nearestCollectable.getPosition());
        }
      }
      if (nearestCollectable != null) {
        distance = 0;
        setDirectionNotCollidingWithOtherPlayersIn2Moves(
            getMap().directionsTowards(playerPosition, nearestCollectable.getPosition()));
      } else {
