...
public class DefendMove extends MoveBase {
  ...
  public DefendMove(GameGeometry map, final Player fullPlayer) {
    super(map, fullPlayer);
    this.tetherDistance = randomTetherDistance();
  }

  public DefendMove(Direction direction, int distance, GameGeometry map, final Player fullPlayer) {
  ...
  private static int randomTetherDistance() {
    ...
    return ThreadLocalRandom.current().nextInt(MINIMUM_SPAWN_POINT_TETHER)
        + MAXIMUM_SPAWN_POINT_TETHER
        - MINIMUM_SPAWN_POINT_TETHER;
  }
  ...
  public void phase() {
    if (spawnPoint != null) {
      ...
      if (playerPosition.equals(spawnPoint.getPosition())
        ...
        direction =
            findBestTetherDirectionOnePositionToAnother(
                playerPosition, spawnPoint.getPosition(), tetherDistance);
      }
      while (getMap()
          .straightLineRoute(playerPosition, direction, 2)
          .collides(myPlayersPositions)) {
        direction = Direction.random();
      }
    } else {
  ...
  private Direction findBestTetherDirectionOnePositionToAnother(
      Position myPosition, Position desiredTether, final int distance) {
    ...
    for (int i = randomModifier; i < DIRECTIONS.length + randomModifier; i++) {
      Position projectedPosition =
          getMap().getNeighbour(myPosition, DIRECTIONS[i % DIRECTIONS.length]);
      int distanceToTether =
          Math.abs(
              findDistanceBetweenTwoPositionsSquared(projectedPosition, desiredTether)
                  - distance * distance);
