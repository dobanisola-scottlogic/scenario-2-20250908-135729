...
public class ProportionalTransitionSelector {
  ...
  public ProportionalTransitionSelector(
      java.util.Map<Class, Integer> moveCounts,
      HashMap<Class, Integer> transitionProfile,
      GameGeometry map,
      MoveBase move,
      Player player)
  ...
  private static MoveBase findMinimumSquaresSolution(
      java.util.Map<Class, Integer> moveCounts,
      HashMap<Class, Integer> desiredProportionsMap,
      GameGeometry map,
      MoveBase move,
      Player player)
    ...
    for (Class outerClazzIteration : moveCounts.keySet()) {
      ...
      for (Class innerClazzIteration : moveCounts.keySet()) {
        if (outerClazzIteration == innerClazzIteration) {
          currentSquaresValue +=
              Math.pow(
                  (moveCounts.get(innerClazzIteration)
                      + 1
                      - desiredProportionsMap.get(innerClazzIteration)),
                  2);
        } else {
          currentSquaresValue +=
              Math.pow(
    ...
    if (moveClass != null) {
      Constructor constructor =
          moveClass.getConstructor(Direction.class, int.class, GameGeometry.class, Player.class);
      return (MoveBase)
          constructor.newInstance(move.getDirection(), move.getDistance(), map, player);
    } else {
