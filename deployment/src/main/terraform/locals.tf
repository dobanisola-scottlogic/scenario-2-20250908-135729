locals {
  ...
  start_schedule = "55 ${var.start_hour - 1} ? * MON-FRI *"
  stop_schedule  = "05 ${var.stop_hour} ? * MON-FRI *"

  # Start/stop the database 5' before/after ECS and EC2
  rds_up_schedule   = "50 ${var.start_hour - 1} ? * MON-FRI *"
  rds_down_schedule = "10 ${var.stop_hour} ? * MON-FRI *"
}
