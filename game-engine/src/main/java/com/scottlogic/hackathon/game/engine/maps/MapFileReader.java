...
/**
 * Given the name of a map, tries to read it from the program's resources, throwing exceptions on
 * failure.
 */
public class MapFileReader {
  ...
  public Arena readMapFile(String mapName) throws MapLoadException {
    ...
    if (json.data != null) {
      ...
      geometry =
          readStringBasedGeometryAndObjects(
              json.map, outOfBoundsPositions, spawnPointPositions, foodSpawnPositions);
    } else {
      throw new MapLoadException(
          "Maps must have either `data` (plus `width` and `height`) or `map` populated");
    }
  ...
  private static GameConfigLayer readConfigFromDto(MapDto json) {
    ...
    if (json.perTurnFoodSpawnProbability != null
        && (json.perTurnFoodSpawnProbability < 0 || json.perTurnFoodSpawnProbability > 1)) {
    ...
    if (json.perTurnFoodSpawnProbability != null)
      configBuilder.setFoodSpawnProbability(json.perTurnFoodSpawnProbability);
    if (json.maximumTurnCount != null) configBuilder.setTurnLimit(json.maximumTurnCount);
    if (json.initialUnitSpawnCount != null)
      configBuilder.setInitialUnitCount(json.initialUnitSpawnCount);
  ...
  private static GameGeometry readArrayBasedGeometryAndObjects(
      MapDto mapDto, Set<Position> outOfBoundsPositions, Set<Position> spawnPointPositions) {
