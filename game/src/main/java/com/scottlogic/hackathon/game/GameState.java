...
/**
 * A 'view' of the state of a game. Instances of this are often not complete, instead corresponding
 * to what a particular {@linkplain Bot}'s {@linkplain Player Players} can 'see'.
 */
@Value
public class GameState {
  /**
   * @return The current phase of the game. The phase starts at 0 and simply counts up during the
   *     game.
   */
  int phase;
   ...
   * @return The player at the requested position, or an {@linkplain Optional#empty() empty
   *     Optional} if there is none
   */
  public Optional<Player> getPlayerAt(Position position) {
    return getPlayers().parallelStream().filter(p -> p.getPosition().equals(position)).findAny();
  }
   ...
   * @return The SpawnPoint at the requested position, or an {@linkplain Optional#empty() empty
   *     Optional} if there is none
   */
   ...
   * @return The Collectable at the requested position, or an {@linkplain Optional#empty() empty
   *     Optional} if there is none
   */
  ...
  /**
   * Determines if the given position is accessible (i.e. not {@linkplain #isOutOfBounds(Position)
   * out of bounds}) and doesn't contain a player, spawn point, or collectable.
   *
   * @param position The position to check
