...
public interface Route extends Iterable<Position> {
  ...
  /**
   * Gets the {@linkplain Direction} that the route moves away from the {@linkplain #getStart()
   * starting position}. If this route has zero length (i.e. is <em>just</em> the starting
   * position), then an {@linkplain Optional#empty() empty Optional} will be returned.
   *
  ...
  /**
   * Gets the sub-route starting from the second position along this route. In other words, this
   * imagines that one step has been taken along this route and returns the portion of the route
   * still to travel. If this route has zero length (i.e. is <em>just</em> the starting position),
  ...
  /**
   * Determines whether this route collides with (traverses) any position that is satisfied by the
   * given predicate.
   *
   * @param obstacles A {@linkplain Predicate} determining whether positions are collision obstacles
   * @return {@code true} iff any of the positions along this route collide with the specified
   *     obstacles
  ...
  /**
   * Determines whether this route collides with (traverses) any of the specified obstacle
   * positions.
   *
   * @param obstacles A {@linkplain Predicate} determining whether positions are collision obstacles
   * @return {@code true} iff any of the positions along this route collide with the specified
  ...
  /**
   * Determines whether this route and the given route collide. Two routes are said to collide
   * <em>only</em> if they traverse the same position at the same 'time'. Formally, two routes,
   * {@code A} and {@code B} are said to <em>collide</em> if there is a position {@code x} and an
   * integer {@code k} such that {@code k}th position along both {@code A} and {@code B} is {@code
   * x}.
   *
