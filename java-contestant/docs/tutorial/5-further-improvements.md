...
- Perhaps the HashMap idea could be extended to be of type `HashMap<Id, YourOwnClass>`, where the objects of your new class could have a direction
...
#### The GameGeometry object

You can access the GameGeometry via `GameGeometry gameGeometry = gameState.getMap();` which also provides a variety of different methods. Some simple methods this provide
include :

- `int height = gameGeometry.getHeight();`
- `int width = gameGeometry.getWidth();`
- `Position position = gameGeometry.getPosition(x,y);`
- `int distance = gameGeometry.distance(p1, p2);`
- `Optional<Direction> direction = gameGeometry.directionsTowards(positionFrom, positionTo).findFirst();`
...
If you want to then get the opposite direction (perhaps to make the players move away from the water), it can be achieved by `Direction opposite = d.getOpposite();`
