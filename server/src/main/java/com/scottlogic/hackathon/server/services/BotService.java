...
public class BotService {
    ...
    private boolean userCanAccessBot(final User user, final UUID id) {
        return user.isAdmin() || (user.isTeam() && isTeamBot(teamService.getTeam(user.getName()), botStore.getByTeamId(id)));
    }

    private boolean isTeamBot(Team team, TeamBot teamBot){
        Predicate<TeamBot> isUserTeamBot = bot -> Objects.nonNull(bot) && bot.getTeamId().equals(team.getId());
        return isUserTeamBot.test(teamBot);
    }

     private List<TeamBot> getUserAccessibleBots(final User user) {
         return botStore.list().stream().filter(b -> userCanAccessBot(user, b.getTeamId())).collect(toUnmodifiableList());
    }

    public Map<Team, Bot> createTeamBotMap(final User user, final GameConfiguration gameConfiguration) {
        final var userAccessibleBots = this
                .getUserAccessibleBots(user)
                .stream()
                .collect(toMap(TeamBot::getTeamId, Function.identity()));

        final var milestoneBots = milestoneService
                .getMilestones()
                .stream()
                .collect(toMap(MilestoneBot::getMilestoneClassName, Function.identity()));

        final var teams = gameConfiguration
                .getTeams()
                .stream()
                .collect(toMap(Function.identity(), this::getTeam));
    ...
    private Bot getTeamOrMileStoneBot(Team team, Map<UUID, TeamBot> teamBots, Map<String, MilestoneBot> milestoneBots) {
