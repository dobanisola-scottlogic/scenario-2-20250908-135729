...
public class AbstractStore<T> {
    ...
    public T get(final Criterion criterion) {
        T entity;
        try {
            entity = (T) currentSession()
                    .createCriteria(entityClass)
                    .add(criterion)
                    .uniqueResult();
        } catch (final HibernateException ex) {
            logger.error(String.format("Error retrieving %s from database", entityClass.getName()), criterion, ex);
            entity = null;
        }
        return entity;
    }
    ...
    public List<T> list(final Criterion criterion) {
        ...
        try {
            ...
            logger.error(String.format("Error retrieving %s list from database", entityClass.getName()), criterion, ex);
