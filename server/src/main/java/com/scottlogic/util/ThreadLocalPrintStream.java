 ...
 *
 * <p>This class's {@link #printf(String, Object...) printf(...)}, {@link #format(String, Object...)
 * format(...)}, and {@link #append(char) append(...)} methods return the current thread's delegate
 * rather than this object. This is for a slight performance improvement, as it is assumed that
 * chained calls to the PrintStream will remain in the same thread, and so the additional cost of
 * looking up the correct delegate each time is unnecessary.
 */
public final class ThreadLocalPrintStream extends PrintStream {

  private final PrintStream defaultDelegate;
  private final ThreadLocal<PrintStream> delegate =
   ...
   *
   * @param defaultDelegate The {@linkplain PrintStream} to delegate to, unless another has been
   *     explicitly {@linkplain #setThreadLocalDelegate(PrintStream) set}
   */
